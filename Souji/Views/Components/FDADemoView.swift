...
struct FDADemoView: View {
    ...
    private var rows: [DemoRow] {
        [
            ...
            DemoRow(name: "Souji", systemImage: "sparkles",
    ...
    private func rowView(_ row: DemoRow) -> some View {
        let highlight = row.isSouji && (frame == .highlight)
        let succeeded = row.isSouji && (frame == .granted || frame == .hold)

        return HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 6, style: .continuous)
                    .fill(row.isSouji ? Tint.blue.opacity(0.16) : Color.primary.opacity(0.08))
                Image(systemName: row.systemImage)
                    .font(.system(size: 13, weight: .semibold))
                    .foregroundStyle(row.isSouji ? Tint.blue : .secondary)
            }
            .frame(width: 26, height: 26)

            Text(row.name)
                .font(.system(size: 13, weight: row.isSouji ? .semibold : .regular))
