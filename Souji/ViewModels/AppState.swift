...
final class AppState: ObservableObject {
    ...
    init(
        ...
        if performStartupTasks {
            ...
            // via onboarding.
            if UserDefaults.standard.bool(forKey: "Souji.OnboardingComplete") {
                scheduler.start()
            }
        }
    }
    ...
    /// originates from Souji itself - TCC then registers Souji in the
    ...
    /// FDA to Souji made no difference (issue #75).
    ...
    private func removalFailureMessage(
        ...
        if needsFullDiskAccess {
            ...
            return "\(prefix) could not be removed because Souji does not have Full Disk Access. Grant Full Disk Access in System Settings, then try again."
        }
    ...
    private func runScheduledScan() async {
        ...
        // reclaims it on its own and Souji does not claim to free it. See
