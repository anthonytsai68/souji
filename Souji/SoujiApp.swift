...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    /// Normally Souji quits when its window closes. When the menu-bar system
    /// monitor is enabled the app stays resident so the meters keep updating in
    /// the menu bar; "Open Souji" in that menu reopens the window.
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        // Register the Finder Services provider so "Uninstall with Souji"
    ...
    /// hands it to AppState through a notification. Brings Souji forward so
...
extension Notification.Name {
    ...
    static let soujiMenuBarMonitorChanged = Notification.Name("Souji.MenuBarMonitorChanged")
}
...
struct SoujiApp: App {
    ...
    @AppStorage("Souji.OnboardingComplete") private var onboardingComplete = false
