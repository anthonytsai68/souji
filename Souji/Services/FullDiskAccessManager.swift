...
/// Detects whether Full Disk Access (FDA) has been granted to Souji.
...
final class FullDiskAccessManager {
    ...
    /// the OS records Souji as the requester and adds it to the FDA pane.
    ...
    /// Force Souji to appear in the Full Disk Access list.
    ///
    ...
    /// *Finder* — not Souji. So at launch we touch a small set of paths
    ...
    /// Souji in unrelated permission lists ("Messages", "Contacts", etc.)
    ...
    /// Reveal the running Souji.app bundle in Finder so the user can drag
    ...
    /// Reset Souji's TCC entries so the OS can re-register the bundle.
